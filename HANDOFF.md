...
| Date       | Feature                                  | Notes |
|------------|------------------------------------------|-------|
| 2026-06-29 | **Connector latch floor threshold (`FLOOR_THRESHOLD`)** (backend worker) | Added `FLOOR_THRESHOLD=0.65` to `llm/tools/connector_intent.py` — a not-yet-latched connector must clear BOTH its per-connector threshold (0.60/0.65) AND the global floor to latch. Rejects weak winners that only "won" because every score was low (e.g. "help me debug" ~0.66 vs drive centroid → above 0.60 but below 0.65 → no latch). Already-latched connectors unaffected. Logged when floor rejects: `[latch] winner=X score=Y below floor=Z`. Docs: backend/root `CLAUDE.md`, `BUGS.md`, `QUEUE.md`, `connector_intent.py` docstring, `stream.py` docstring; `graphify update .`. |
