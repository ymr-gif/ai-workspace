import s, { AMBER, FG3, FG4, FG5 } from '../lib/chatStyles.js'

export default function ParamSlider({ label, enabled, onToggle, value, onChange, min, max, step, fmt }) {
  return (
    <div style={s.paramItem}>
      <input type="checkbox" checked={enabled} onChange={e => onToggle(e.target.checked)} style={{ accentColor:AMBER, cursor:'pointer' }} />
      ...
      <input type="range" min={min} max={max} step={step} value={value} disabled={!enabled}
        ...
        style={{ flex:1, accentColor:AMBER, cursor: enabled ? 'pointer' : 'default' }} />
