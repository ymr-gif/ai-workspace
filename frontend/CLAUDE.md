...
- **Integrations panel** — Integrations header button; slide-in; `useIntegrations.js`; sources list from `GET /api/integrations`; per-card status badge, sync button (`POST /api/integrations/{id}/sync`), delete (`DELETE`); as of 2026-06-29 `ENABLED_CONNECTOR_TYPES = ['google_drive','google_calendar','gmail']` — **Drive/Calendar/Gmail render with a live OAuth button** (`startOAuth()`, popup-closed polling, popup-blocked banner, error banner). Re-enabled so admin can drive real traffic through the connector-intent latch (Phase 0 data collection, `plans/connector-latch-data-plan.md`). **Notion + GitHub still render as "More integrations on the way"** with a "Soon" badge (opacity 0.6, no OAuth button). To expose/re-stub a connector: add/remove its type in `ENABLED_CONNECTOR_TYPES`.
