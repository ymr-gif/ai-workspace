...
- **Integrations panel** — Integrations header button; slide-in; `useIntegrations.js`; sources list from `GET /api/integrations`; per-card status badge, sync button (`POST /api/integrations/{id}/sync`), delete (`DELETE`); as of 2026-07-02 `ENABLED_CONNECTOR_TYPES = []` (**RE-STUBBED**) — **all connectors incl. Drive/Calendar/Gmail render as "More integrations on the way"** with a "Soon" badge (opacity 0.6, no OAuth button). They were exposed with a live OAuth button 2026-06-29 → 2026-07-02 for connector-intent latch data collection; re-stubbed once the tuning closed (fork-B, `plans/connector-latch-data-plan.md` → Phase 4 DECIDED). The OAuth-button rendering path (`startOAuth()`, popup-closed polling, popup-blocked banner, error banner) stays in the component — re-expose a connector by re-adding its type to `ENABLED_CONNECTOR_TYPES`.
