...
- **OAuth connectors (Drive + Calendar + Gmail) — UI-stubbed as of 2026-06-28:** `ENABLED_CONNECTOR_TYPES = []` in the frontend; no users can OAuth into these connectors. The capability gates (`_drive_gate`/`_cal_full_gate`/`_gmail_full_gate`) and the session intent latch (`connector_intent.py`) are backend-complete but never fire (no active connector rows can exist). If connectors are re-enabled, the latch applies: connector schemas are withheld until embedding-cosine intent latches the session (single-winner across connectors); see connector injection bullets below + `llm/tools/connector_intent.py`.
...
- **`LLM_BACKEND` switch (`nim`|`homeserver`, default `nim`)** — single flag in `config.py` that flips the app between the NIM test backend and the local llama.cpp/Mixtral stack. A gated block (run **before** the startup guards, so `importlib.reload` re-applies it) repoints `NIM_URL`/`NIM_EMBEDDING_URL`, collapses `MODELS` to one Mixtral alias, sets `CONTEXT_WINDOWS`/`DEFAULT_CONTEXT_WINDOW=32768`, swaps `MODEL_EMBEDDING`→`bge-large-en-v1.5` (1024-d, no re-embed), and relaxes the `NVIDIA_API_KEY` guard (`LLM_BACKEND != "homeserver"`). Toggleable live via `/admin/env` PUT + reload — **no restart**.
  ...
  - ⚑ **Connector-intent latch centroids are embedder-specific — re-tune on the bge swap (MOOT WHILE STUBBED).** Connectors are currently UI-stubbed (`ENABLED_CONNECTOR_TYPES = []`); the latch never fires. If connectors are re-enabled: `llm/tools/connector_intent.py` derives a per-connector centroid (drive/calendar/gmail) at boot from `INTENT_PHRASES`; those auto-regenerate correctly under bge. BUT `INTENT_THRESHOLDS` (drive 0.60 / calendar 0.60 / gmail 0.65) are tuned to nv-embedqa-e5-v5's score distribution and will be **wrong** for `bge-large-en-v1.5`. On the homeserver port: re-run `tests/{drive,calendar,gmail}_intent_eval.jsonl` under bge and re-set all three thresholds (+ recheck cross-talk). Symptom if skipped: connector schemas latch too early/late silently. (Q3 Task B.)
