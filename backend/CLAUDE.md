...
- Drive tool injection (`llm/tools/builtin/drive_tools.py:_drive_gate`): all three Drive tools offered whenever `ctx.drive_active` (connector connected). No keyword/cache pre-filter — the model picks list/search/read from the schemas; `_DRIVE_RULES` behavioral block steers the list→ask→read flow. `_drive_list_files`/`_drive_search` are still `is_list_tool` (loop-abort after 1 identical call).
