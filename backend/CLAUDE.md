...
│   └── latch/ — connector-intent latch data-collection harness (Phase 0→2): agent_capture.py (API) + ui_capture.py (Playwright UI twin) write labeled capture rows; measure.py joins them with `latch_score` logs → outputs A/B/C + emits eval sets; prompt_bank.py (hard-band prompts); README.md. Runbook: `plans/latch-data-session-kickoff.md`. Also hosts the full-feature exercisers: rich_exercise.py (agent loop + connectors) + **rich_full.py / run_rich_full.sh** (whole documented surface incl. real mutations; last run 2026-07-03 — report in rich_full_logs/)
├── llm/
...
│   │   ├── router.py       — POST /chat (non-streaming; cost-cap pre-flight + spend recorded via usage_ledger)
│   │   ├── usage_ledger.py — stateless-endpoint spend metering (QUEUE Q4, 2026-07-03): /chat + /v1/chat/completions record assistant rows into one hidden archived "[API usage]" conversation per user — the messages-rows ledger (cost cap + /usage) includes them automatically; never raises into the reply path
...
│   │   └── memory.py       — POST /memory/reset (soft/hard); _soft_reset includes _purge_canvas_entities() Neo4j cleanup; GET /memory/versions?user_id= (snapshot list) + POST /memory/restore {user_id,version_id,confirm:"RESTORE <user_id>"} (reversible rollback — snapshots current sheet before overwriting from a user_memory_versions row; audits memory.restore; verified live 2026-07-03)
...
- **OAuth connectors (Drive + Calendar + Gmail) — UI RE-STUBBED as of 2026-07-02:** `ENABLED_CONNECTOR_TYPES = []` in the frontend removes the OAuth button so no NEW user can connect. **Note it does NOT deactivate existing connections** — `_{drive,calendar,gmail}_active` read from DB `ExternalSource` rows, not the frontend array, so admin's rows from data collection stay `active` and the latch KEEPS firing on admin turns. Fully deactivating (latch off for everyone) requires deleting/pausing those rows. They were EXPOSED 2026-06-29 → 2026-07-02 for **Phase 0 latch score-logging data collection** (`plans/connector-latch-data-plan.md`); enough data was collected and the tuning CLOSED (fork-B: `FLOOR_THRESHOLD=0.70` + clarify fallback — Phase 4 DECIDED). The mechanics below describe the latch as built and are **live-relevant only when connectors are re-exposed** (e.g. on the home-server/bge stack). The capability gates (`_drive_gate`/`_cal_full_gate`/`_gmail_full_gate`) + latch withhold each connector's schemas until embedding-cosine intent latches the session (single-winner across connectors); see connector injection bullets below + `llm/tools/connector_intent.py`. **Phase 0 instrumentation (still in code):** `_resolve_connector_latches` (`llm/service/stream.py`) scores all three connectors every turn and emits one JSON line per turn via logger `connector_intent.scores` (`evt:"latch_score"` — conv_id/turn/query_emb_present+reason/scores/prior_latch_state/active/argmax/runner_up/margin/decision/why/msg). **`INTENT_THRESHOLDS`/`FLOOR_THRESHOLD` are `nv-embedqa-e5-v5` geometry — re-run eval sets + re-tune on the bge swap (do NOT port the numbers).** Cache-hit turns return before the latch → no log line.
...
- **`LLM_BACKEND` switch (`nim`|`homeserver`, default `nim`)** — single flag in `config.py` that flips the app between the NIM test backend and the local llama.cpp/Mixtral stack. A gated block (run **before** the startup guards, so `importlib.reload` re-applies it) repoints `NIM_URL`/`NIM_EMBEDDING_URL`, collapses `MODELS` to one Mixtral alias, sets `CONTEXT_WINDOWS`/`DEFAULT_CONTEXT_WINDOW=32768`, swaps `MODEL_EMBEDDING`→`bge-large-en-v1.5` (1024-d, no re-embed), and relaxes the `NVIDIA_API_KEY` guard (`LLM_BACKEND != "homeserver"`). Toggleable live via `/admin/env` PUT + reload — **no restart**.
  ...
  - ⚑ **Compose env outranks `.env`** (found 2026-07-03): for keys the compose environment sets (`WEB_SEARCH_ENABLED`, `MODEL_*`, …), `/admin/env` PUT changes the *running* config (setattr) but `POST /admin/env/reload` or a container restart re-reads `os.environ` and **re-masks the `.env` value**. Corollary: `MODEL_*` flips never reach routing at all (`MODELS` frozen at import in `api/chat/helpers.py`) — changing a routed model id requires editing the compose env + recreate, not `/admin/env`.
...
- Circuit breaker: _THRESHOLD=5, _COOLDOWN=90s; Redis-persisted `cb:open:{model}` EX 90; restored on startup via `restore_circuit_state()`; pre-tripped at startup by `probe_models_on_startup()` for any model returning non-200 (probe never *clears* healthy models). **Enforcement reads in-process state only** (`is_open` in `llm/nim.py:37/153`) — the Redis key is persistence, consumed at startup; setting it on a running process does nothing. Full lifecycle (organic 429 trip → fallback → recovery, + seeded-key restore/enforcement) verified live 2026-07-03
