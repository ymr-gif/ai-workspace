...
- **Exception — OAuth connectors: Drive + Calendar + Gmail (Q3 Task B, since 2026-06-28):** these
  connector tools are gated on capability **AND** a per-connector session intent latch
  (`ctx.{connector}_active AND ctx.{connector}_latched`), not capability alone — each connector's schema
  is withheld until embedding-cosine intent for THAT connector latches the session (fixes the greeting
  over-fire where the 70B fired `drive_list_files` / `calendar_list_events` / `calendar_search_events
  {"query":"hi"}` on greetings). `_drive_gate` / `_cal_full_gate` / `_gmail_full_gate` are no longer
  *pure* capability predicates; the latch flags are resolved together in `generate_stream` before the
  injection loop (single-winner — see below) and threaded via `ToolContext`. See the connector injection
  bullets below + `llm/tools/connector_intent.py`. All non-connector tools remain pure-capability.
...
- **`LLM_BACKEND` switch (`nim`|`homeserver`, default `nim`)** — single flag in `config.py` that flips the app between the NIM test backend and the local llama.cpp/Mixtral stack. A gated block (run **before** the startup guards, so `importlib.reload` re-applies it) repoints `NIM_URL`/`NIM_EMBEDDING_URL`, collapses `MODELS` to one Mixtral alias, sets `CONTEXT_WINDOWS`/`DEFAULT_CONTEXT_WINDOW=32768`, swaps `MODEL_EMBEDDING`→`bge-large-en-v1.5` (1024-d, no re-embed), and relaxes the `NVIDIA_API_KEY` guard (`LLM_BACKEND != "homeserver"`). Toggleable live via `/admin/env` PUT + reload — **no restart**.
  ...
  - ⚑ **Connector-intent latch centroids are embedder-specific — re-tune on the bge swap.** `llm/tools/connector_intent.py` derives a per-connector centroid (drive/calendar/gmail) at boot from `INTENT_PHRASES`; those auto-regenerate correctly under bge. BUT `INTENT_THRESHOLDS` (drive 0.60 / calendar 0.60 / gmail 0.65) are tuned to nv-embedqa-e5-v5's score distribution and will be **wrong** for `bge-large-en-v1.5` (same 1024-d, different geometry). On the homeserver port: re-run `tests/{drive,calendar,gmail}_intent_eval.jsonl` under bge and re-set all three thresholds (+ recheck cross-talk, since the single-winner margin shifts too). Symptom if skipped: connector schemas latch too early/late silently. (Q3 Task B.)
