...
- `[x]` **70B streaming fails with `stream_network_error attempt=0` → silent fallthrough to deepseek** — **FIXED 2026-07-02.** Root cause = solution 2: the shared client is built `httpx.AsyncClient(timeout=REQUEST_TIMEOUT)` (`main.py:72`), and a bare float sets the **read** timeout to 30s. On a stream the read timeout is time-to-first-token; the 70B (large model + tool schemas) exceeds 30s before the first token → `httpx.ReadTimeout` (`stream_network_error attempt=0`) → retries hit the same wall → raise → fallback to coder(deepseek). deepseek's fast TTFB never trips it, hence "why deepseek not 70B." **Fix:** new `STREAM_READ_TIMEOUT` (default 120s, `config.py`) applied per-request in `call_stream` via `timeout=httpx.Timeout(REQUEST_TIMEOUT, read=STREAM_READ_TIMEOUT)` (`nim.py`) — connect/write/pool stay at 30s, only the first-token/inter-token read window widens. Not solutions 1/3: the retry loop already covered transport errors, and the breaker was tripping legitimately (every attempt genuinely timed out); widening the read window removes the timeouts at the source. (original detail below)
  - **Symptom:** a turn routes `route → meta/llama-3.3-70b-instruct` then emits ONLY `status` events (memory/classify/embed/route/budget) and NO tokens; the client sees a stalled/empty stream until it times out. API log shows `[nim] stream_network_error model=meta/llama-3.3-70b-instruct err= attempt=0`. User-visible as "why are most replies coming from deepseek, not the 70B" — the 70B keeps failing so the fallback chain (`chosen → reasoning(70B) → coder(deepseek) → llama`) lands on the coder.
  - **Distinct from** the embedder 500-storms (those null `query_emb` → `reason: embed_fail` in `latch_score`; this is the *chat completion* stream on the 70B specifically, embedder healthy). Also distinct from a tripped circuit breaker — this is the network error that (repeatedly) trips it.
  - **Files:** `backend/llm/nim.py` (streaming call / `call_stream` + retry loop; `stream_network_error` log site); `backend/llm/circuit_breaker.py` (5 failures → open, 90s cooldown); fallback chain in `backend/llm/service/stream.py`.
  - **Solutions to investigate (ranked):**
    1. **Confirm the stream path retries transport errors** the way `embed()` was fixed 2026-06-30 (`embeddings.py` `_MAX_ATTEMPTS=4`, retry 5xx+transport). If `call_stream` treats a mid-stream network drop as a hard fail on attempt 0, add the same bounded retry before counting a circuit failure.
