...
- `[ ]` **`write_memory` fires on greetings / trivial turns → empty reply + spurious "save to memory?" card + memory pollution** (found 2026-06-28, live; now the most visible greeting over-fire after the connectors were latch-gated)
  ...
  - **Fix directions (not built):** (1) gate `write_memory` on a memory-intent signal — reuse the `connector_intent` single-winner latch with a "remember this / my preference is …" centroid, or a cheap explicit-intent check; (2) make the confirm path always emit a short text reply so a misfire never yields an empty bubble; (3) stop landing trivial turns on the 70B (revisit the router + `_needs_memory_tool`).
  - **Repro:** send `hello?` repeatedly under admin; when a turn routes to the 70B (watch the `route` SSE / `route_reason`), `write_memory` fires and the saved `messages.content` is empty (confirm via `tool_call_logs`). Delete the test conversation after — it pollutes history + memory.
- `[ ]` **Connector intent-latch — residual leaks (Drive / Calendar / Gmail)** (open follow-ups to the shipped latch; `llm/tools/connector_intent.py`) — the latch makes the *cold* greeting case structurally impossible, but three weaknesses remain: **(1) post-latch trivial turns** — once a connector is latched, `thanks`/`ok` can still fire its tools (schema legitimately present; the per-connector `_RULES` prompt is the only guard and stays weak on the 70B). **(2) sticky-latch session-poisoning** — the latch is sticky 1h and the centroid can't cleanly separate connector-intent from *substantive task/coding imperatives* ("help me debug" ≈ 0.66 vs the drive centroid; gate accuracy ~32/40), so a false latch on a coding turn flips a connector on for the rest of the session (single-winner caps the blast radius at **one** connector, not all three). **(3) verification rigor** — the cold-case tests are structural/deterministic (firm), but the probabilistic 70B behavioral tests were run once each, not the 3–5 repeats originally specced. Mitigations to weigh: raise `INTENT_THRESHOLDS` (now drive 0.60 / calendar 0.60 / gmail 0.65), decay/non-sticky latch, a 2-class/contrastive gate, or an 8B post-latch pre-pass. Shipped record: `HANDOFF.md` → "COMPLETED PHASE: Q3 Task B" + the two 2026-06-28 History rows.
