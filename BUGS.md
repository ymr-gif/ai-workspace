...
- `[x]` **Calendar + Gmail tools fire on greetings / trivial turns — same capability-only over-fire as Drive** (found 2026-06-28; **FIXED + verified live 2026-06-28** via the generalized connector latch, root override) — the connector tools were injected on capability alone, so the tool-eager 70B called them unprompted on greetings/chit-chat. **Same root cause + same class as the Drive bug below** (now fixed via the Q3 Task B latch): `_cal_full_gate` (`llm/tools/builtin/calendar_tools.py:23`) = `return ctx.calendar_active` and `_gmail_full_gate` (`gmail_tools.py:21`) = `return ctx.gmail_active` — both pure-capability predicates, exactly the old `_drive_gate`. **Evidence (live, 2026-06-28):** surfaced as a side effect of the Drive Task B behavioral run — `ehllo` and `hello?` (Drive now schema-gated, no Drive fire) both fired `calendar_list_events` instead; `tool_call_logs` also shows `calendar_search_events {"query":"hi"}` (calendar searched for the greeting word "hi"). **Scope:** Calendar = 6 tools (list/search/get leak freely; create/update/delete are behind the `__CONFIRM_CALENDAR_WRITE__` card so noisier-not-dangerous). Gmail = 3 tools, **latent** — only because Gmail isn't connected for admin; over-fires the moment it is. **Fix SHIPPED + verified live 2026-06-28 (root override):** the Drive latch was generalized into `llm/tools/connector_intent.py` (per-connector centroids + thresholds: drive 0.60 / calendar 0.60 / gmail 0.65; `drive_intent.py` deleted). Each connector now gates on `{connector}_active AND {connector}_latched`; flags resolved together in `generate_stream._resolve_connector_latches`. **Cross-talk handled — single-winner:** the connectors share a "check my X" structure (measured: a gmail request scores ≥0.60 on the drive centroid 13/20, calendar 12/20), so independent latching would let one request latch all three. The flip latches only the **top-scoring** active+unlatched connector (the correct one is the argmax ~always), shrinking a false latch to one connector instead of all. **Live (admin, drive+calendar active, 11/11 checks):** `ehllo` → `tools=[]` (no calendar fire — was firing `calendar_list_events`); calendar request → `calendar_list_events` fires + calendar latched + **drive NOT latched**; drive request → `drive_list_files` fires + drive latched + **calendar NOT latched**. Gmail not live-testable (not connected for admin) — covered by unit tests (`test_gmail` gating requires `gmail_latched`) + structural (`gmail_active=False` → never injected) + the eval (gmail separates cleanly @0.65). **Residual (same as Drive):** post-latch trivial-turn leak + task-imperative false-positive band (now blast-radius 1 connector via single-winner) + sticky-1h latch — see the Drive entry's residual list. Generalized eval sets: `tests/{calendar,gmail}_intent_eval.jsonl`.
